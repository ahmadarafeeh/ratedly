// This screen is for viewing the profile of another
// user. It includes functionality to follow/unfollow the user,
//  block the user, and navigate to a messaging screen.
...
import 'package:helloworld/widgets/UserRatingSection.dart';
...
import 'package:helloworld/widgets/UserListScreen.dart';
import 'package:helloworld/widgets/user_rating_list_screen.dart';
import 'package:helloworld/resources/block_firestore_methods.dart'; // Add this
...
class _OtherUserProfileScreenState extends State<OtherUserProfileScreen> {
  ...
  Future<void> _otherGetData() async {
    ...
    try {
      ...
      // Redirect if either side blocked
      // In the _checkBlockStatus method:
          ...
          Navigator.pushReplacement(
            ...
            MaterialPageRoute(
              builder: (context) => BlockedProfileScreen(
                uid: widget.uid,
                isBlocker:
                    isBlockedByMe, // Correctly pass if current user is blocker
              ),
            ),
          );
        }
      ...
      final totalRating = ratings.fold<double>(
          0.0, (sum, r) => sum + (r['rating'] as double? ?? 0.0));
      final averageRating =
  ...
  Widget _buildFollowButton(bool isPrivateAccount) {
    ...
    return ElevatedButton(
      ...
      style: ElevatedButton.styleFrom(
        ...
        side: BorderSide(
          color: const Color(0xFF333333), // Match background
        ),
        minimumSize: const Size(100, 40),
      ),
      child: Text(
  ...
  Widget _buildOtherBlockedContainer() {
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 16),
          const Text("This user is blocked",
              style: TextStyle(fontSize: 18, color: Colors.grey)),
          const SizedBox(height: 20),
          ElevatedButton(
            onPressed: () async {
              await FirestoreBlockMethods().unblockUser(
  ...
  Widget _buildPrivateAccountMessage() {
    return Column(
      ...
      children: [
        ...
        const Text('Follow to see their posts', style: TextStyle(fontSize: 14)),
      ],
    );
  }
